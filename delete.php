...
<?php
		...
		function edugorilla_client_delete()
    {
    	...
    	if($pl_delete_form == "self")
        {
	        $wpdb->delete($wpdb->users, array('id' => $iid));
			$wpdb->delete($wpdb->prefix . 'edugorilla_client_preferences', array('id' => $iid));
...
?>
	<div class="wrap">
		<h1>Client List</h1>
    <form method="get" action="admin.php">
    	<input type="hidden" name ="page" value="edugorilla-delete-client">
    	<input type=hidden name="iid" value="<?php echo $iid; ?>">
    	<table class="widefat fixed" cellspacing="0">
			<tbody>
				<tr>
					<th>Do you want to delete <?php echo $client_name; ?> ?</th>
				</tr>
            	<tr>
